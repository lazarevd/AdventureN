...
public class MoveWork extends Work {
	...
	public MoveWork (Vector2 xy, GameLevel gameLevel) {
		super(gameLevel);
		this.xy = xy;
	}

	public MoveWork (Thing toThing, Vector2 defaultPosition, GameLevel gameLevel) {
		super(gameLevel);
		NodeGame nod = gameLevel.getGraph().getNodes().get(toThing.getNodeName());
	...
	public void init() {
		...
		gameLevel.getGraph().addStartFinishNodes(new Vector2(mActor.getX(), mActor.getY()), xy, mActor.getRenderScale());
